...
#
# 60분인 이유는 아래가 전부다. 실제 세션에서 관측된 최대 공백이 31분이고
# (20260728·29의 몇 종목, 정확히 한 페이지 분량이라 조회 누락으로 의심된다 — 별도
# 확인 대상), 정상적인 날을 매번 다시 받지 않으려면 그 위여야 한다.
#
# **이 검사가 위 버그를 전부 잡지는 못한다.** 잘린 조회가 아침 조각에 가깝게까지
# 받아왔다면 남는 공백이 21분일 수도 있고, 그런 기록은 여기를 통과한다. 다만 그때는
# 실제로 빠진 구간도 그만큼 짧아 워밍업 값이 흔들리지 않는다. 이 검사가 막는 것은
# 세션의 한 덩어리가 통째로 없는 경우다.
_MAX_GAP_MINUTES = 60
...
def _has_hole(times: list[str]) -> bool:
    ...
    단일가 구간(15:20~15:30)은 비어 있는 것이 정상이라 제외하지만, **그 앞까지는
    반드시 이어져야 한다.** 제외만 하고 끝내면 "오전만 촘촘하고 종가 한 봉"이
    통과한다 — 가운데가 두 시간 넘게 비어 있는데도 양 끝은 닿기 때문이다.
    """
    trading = [_minutes(t) for t in times if t <= _CONTINUOUS_END]
    if not trading:
        return True
    if _minutes(_CONTINUOUS_END) - trading[-1] > _MAX_GAP_MINUTES:
        return True
    return any(
