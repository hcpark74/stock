...
F1이 후보를 못 찾은 것도(9개 후보를 찾아 실제 체결까지 이어졌다) 아니다. **"구독은 했는데 틱이
0건"** 행에 해당한다 — 다만 원인은 WS 미수신이 아니라, F3가 최종 픽을 F1/F2 잠금 종목과 다르게
바꿨을 때 `tick_capture`가 새 종목으로 전환되지 않아 틱이 전량 필터링된 것이다.

이 실패 모드는 우연이 아니라 구조적이다. F3가 rank-1 잠금 종목을 갭 재검증에서 탈락시키고 하위
랭크로 넘어가는 날마다(08-26이 정확히 그 경우) 재현될 수 있다. 그리고 실제로 체결·청산까지 끝난
215600에는 `strategy_ticks` 파일도 `price_path_manifests` 행도 전혀 남지 않았다 — 그날 실제로
거래한 종목의 가격 경로가 통째로 유실된 것이다. 1단계("봉이 매일 쌓인다")의 표본은 이런 날마다
비게 된다. **1단계 봉 적재를 시작하기 전에, `tick_capture`가 F3의 최종 전환을 따라가도록 고치는
작업(F4가 캡처 대상을 재확인·재부착하게 하거나, F3가 최종 픽을 확정하는 지점에서 명시적으로
재전환을 트리거하는 것)을 선행 태스크로 추가한다.**
