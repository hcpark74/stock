...
| 계좌 분리 | 하지 않는다. 단일 계좌 |
| 종목 | ~~두 트랙이 같은 종목~~ → **이탈**: A는 F3가 확정한 종목, B는 09:35에 F1 랭크 1~5
중에서 직접 고른다 — §2.2 |
...
### 2.2 종목 이탈 — B가 F1 랭크 1~5 중 직접 고른다

원래 확정한 "두 트랙이 같은 종목을 매매한다"에서 이탈한다. **A는 F3가 확정한 종목을 그대로
쓰고, B는 09:35에 F1 랭크 1~5 중에서 직접 고른다.** F2 락업(`F2_MAX_TARGET_CANDIDATES=3`)이
3개이므로, 랭크 4~5는 A가 그날 시도조차 하지 않는 종목이다. (B는 운영 F1 랭킹
`f1_selector.rank_candidates`를, A의 백테스트 하네스는 자체 정책 랭킹을 쓰며 23일 중
7일에서 상위 5개가 다르다 — "랭크 4~5 = A의 시도 밖"은 대략적 대응이다.
[트랙 B 매매 규칙 선정 — 관문 판정](../../TRACK_B_RULE_SELECTION_20260828.md) §5 참고.)

**사유** — B의 가설은 "개장 급등이 한 번 식은 뒤의 복귀를 09:35 이후에 잡는" 시간 분산이다
(트랙 B 매매 규칙 선정 설계 §1). A와 같은 종목 하나로 묶으면 A가 이미 진입해 점유한
종목만 볼 수 있어 시간 분산의 표본이 A의 진입일에 종속된다. 랭크 1~5로 넓히면 A의 시도
범위 밖 종목도 B의 표본에 들어온다.

**대가** — 이 이탈에는 공짜가 없다.

- **호가 스프레드를 신호 근거로 남길 수 없다.** 그림자 스펙 §8.3은 판정 근거로 신호 시점의
  최우선 호가 스프레드를 함께 남기라고 했는데, 그 값은 틱에서만 나온다. B가 A와 다른
  종목을 고른 날에는 그 종목의 틱이 구독되지 않으므로 `shadow_trades`에 `null`로 남기고
  사유를 함께 적는다 — 감춘 채 0으로 채우면 나중에 재해석이 오염된다.
- **후보 종목의 봉은 틱이 아니라 REST로 받는다.** 09:35에 다섯 종목을 비교하려면 그 종목의
  09:00~09:35 봉이 필요하다. 틱 팬아웃으로 다섯 종목을 전부 구독하면 A의 틱 경로에 비용이
  실려 그림자 스펙 §13의 무간섭 측정이 깨진다. 대신 09:35에 당일분봉 REST를 1회
  호출한다(5종목 × 2페이지 = 10콜, PAPER 1.1초 간격 약 11초). A가 HOLDING이고 WS가
  stale이면 조회를 미루고, 그날은 B가 진입하지 않는다 — A의 안전이 B의 기회보다 앞선다.

세부 설계는 [트랙 B 매매 규칙 선정 설계](2026-08-28-track-b-rule-selection-design.md) §6에
있다.
...
§3.4·§6.2·§7·**§8.3**을 다루고, §5·§6.3~§6.5·§8.2는 그 다음(PILOT)으로 미룬다. 그 스펙은
두 단계로 나뉜다.

1. **봉/지표 계층 + 봉·지표 차트(§8.3)** — 09시 급등주의 분봉을 눈으로 본다
2. **신호 엔진 + 그림자 기록(§6.2)** — 1단계에서 본 것으로 규칙을 정해 얹는다

§8.3(지표 차트)이 §6.2(그림자 기록)보다 **먼저** 온다. 규칙의 숫자를 근거 없이 박고 한 달치
그림자를 쌓은 뒤에 눈으로 보는 순서를 피하기 위해서다. §7.4가 지표를 순수 함수로 만든 덕에
봉만 쌓여 있으면 규칙을 나중에 정해 소급 재생할 수 있어, 이 순서가 표본 시계를 늦추지 않는다.

또 그 스펙은 §7.2의 "`live._accumulate_minute`를 OHLCV로 확장한다"에서 의도적으로 이탈한다.
`src/live.py`가 `_STRATEGY_FILES`에 있어 손댈 때마다 트랙 A의 지문이 돌기 때문이다.
