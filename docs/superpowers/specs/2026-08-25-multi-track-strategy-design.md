...
> **상태**: 설계 확정 — 트랙 모델·DB 스키마·집계 스코프(§3·4·6.1) 구현 완료, 그림자·승격 대기
> **작성일**: 2026-08-25
> **갱신일**: 2026-08-26 (§4 DB 마이그레이션 리허설 완료·§12 갱신, 지문 회전 `f864e8a95ba2`→`d4435896a8a2`)
...
| `7f5362a` | trades 재작성 — `track` 컬럼 + `(date, track)` UNIQUE | §4.1·§4.3 |
...
완료: §3.1·§3.2(트랙 모델·영속화), §4(DB 스키마·마이그레이션), §6.1(집계 쿼리 트랙 스코프).

미착수: §5(예산·불변식), §6.2~§6.5(그림자·승격), §7(봉/지표), §8(트랙 UI).
...
`strategy_fingerprint()`는 `src/db.py`를 포함한 전략 파일 해시라 이번 마이그레이션 코드
자체가 지문을 회전시킨다: `main`의 `f864e8a95ba2` → 이 브랜치의 `d4435896a8a2`. 배포 후
`experiment_id`는 `baseline-d4435896a8a2`로 바뀐다. **이전 실험의 40거래일 paired 수집은
여기서 끊기고 0부터 다시 시작한다** — Plan 3의 그림자 승격 기록은 이 새 실험 ID 위에서
쌓인다.
