...
**재검토 중(2026-08-31).** 랭크 1~5의 팬아웃이 규칙의 선택성을 무력화한다는 것이
실측됐다 — R1의 종목 단위 발화율은 36.4%인데 다섯 종목을 동시에 보면 일 단위 진입율이
90.9%가 된다. 랭크 1로 줄이면 선택성이 돌아오지만 A 미진입일 커버가 3일에서 1일로
줄고 판정 표본이 절반 이하가 된다. **이 절(랭크 1~5)은 재판정 전까지 유효하다** —
재판정 조건과 근거는 [트랙 B 종목 유니버스 재분석](../../TRACK_B_UNIVERSE_REANALYSIS_20260831.md)
§8·§9에 있다.
