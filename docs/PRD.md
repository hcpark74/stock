[PRD] 데일리 1종목 타겟 시가 갭 돌파 자동매매 시스템 (11시 All-Out 버전)

문서 버전: v1.3
작성일: 2026년 6월 23일
최종 수정: 2026년 6월 29일 (F0 Macro Kill Switch 기준 추가)
프로젝트 목적: 한국 증시 개장 직후(08:40~11:00)의 유동성 프리미엄을 포획하고, 11시
...
● 최우선 로직: 11시 정각 청산 트리거는 메인 스레드 부하와 무관하게 독립된 비동기 루프에서
  최우선 실행되어야 함.
...
● [08:59:40] 진입 직전 갭 재검증:
  ...
  예상 시가 재조회 후 Gap 재계산.
  Gap < 0.020 또는 Gap >= 0.065 이면:
    (주문 전 상한 0.065는 체결 상한 0.070과의 0.5%p 차이를 시장가 슬리피지 버퍼로 확보)
...
● 1차 체결 확인 (08:59:50 ~ 09:00:00, 1초 간격 폴링):
  체결 확인 시:
    슬리피지 가드: 체결가 기준 갭 >= 0.070 (체결 상한) 이면
...
──────────────────────────────────────────────────
F4. 장중 추적 스탑 모듈 (09:00:00 ~ 10:59:59)
──────────────────────────────────────────────────
...
● 10:59:50 — Pre-Check:
  ...
  (state와 별개로 저장하는 이유: 11:00:00 직전 F4 청산이 완료될 수 있으므로
...
● Retry 정책:
  ...
  2초 간격으로 최대 3회 재시도.
  최대 지연: 11:00:00 + (2s × 3) = 11:00:06.
  이 지연은 허용 가능한 범위로 간주함. (장 마감은 15:30 / 11시 이후도 거래 가능)
...
● 모니터링:
  장중 09:00~11:00 구간 1분 간격으로 현재 포지션 상태를 콘솔/로그에 출력.
...
● 단축 거래일 (명절 전날 등):
  장 마감 시각이 11:00 이전이면 당일 거래 스킵.
  장 마감 시각이 11:00 이후이면 정상 운영 (F5 청산은 여전히 11:00 기준).
...
● 메시지 포맷:
  ...
  11시 청산 실패! 수동 청산 필요. 005930 200주
  2026-06-23 11:00:06 KST
...
──────────────────────────────────────────────────
  ...
  10:50 ~ 10:59      | 보유   | 복구 시도 없이 즉시 청산 시도 → 실패 시 긴급 알림
  11:00 이후         | 잔류   | 무조건 긴급 알림, 수동 청산 요청
...
● 포지션 보유(HOLDING) 중 NETWORK_DOWN:
  연결 복구 대기 (최대 60초).
  60초 내 복구 시: 정상 재개.
  60초 초과 또는 10:50 이후: 네트워크 복구 즉시 시장가 청산.
  복구 전 11:00 도달 시: 복구 후 첫 번째 동작으로 즉시 청산 + TIMEOUT_ORDER_FAILED 알림.
...
● 개별 종목 VI 발동:
  ...
  처리:
    ...
    VI 해제 후 11:00 이전: F4 추적 재개.
    VI 해제 후 11:00 이후: 즉시 시장가 청산.
    VI 발동 중 11:00 도달: VI 해제 즉시를 첫 번째 동작으로 청산.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  ...
  10:50~10:59   | F4 — Step Trailing 강제 활성화 (스텝 미달성 시 진입가-1.5% 발동)
  10:59:50      | F5 Pre-Check — 잔고 조회 및 prefetch_qty 저장
  11:00:00      | F5 Execute — 미청산 물량 시장가 청산 (Retry 최대 3회)
  11:00:00+     | 로그 최종 기록, today_state.json CLOSED 갱신, daily_pnl_pct 갱신, 알림 발송
---
...
- F3 예정 시각이 이미 지난 경우에는 F3를 `force=True`로 실행해 내부 시각 대기를 건너뛴다.
- F3 체결 확인 마감 이후에는 기본 catch-up 진입을 금지한다. 운영자가 명시적으로 `FORCE_CATCHUP=1`을 설정한 경우에만 강제 보완을 허용한다.
...
- `주문` 메뉴는 주문 실행과 처리 결과를 담당한다.
  ...
  - 원천 데이터는 `/api/orders`가 반환하는 `orders` 테이블 조회 결과이며, 주문 관련 JSONL 이벤트 로그는 즉시 갱신 신호와 진단 보조 정보로 사용한다.
...
- `오늘` 메뉴의 보유 후 가격흐름 차트는 `/api/status.tick_history`의 tick `ts` 값을 x축으로 사용한다.
  - x축 라벨은 `HH:mm:ss` 형식이다.
  - 짧은 보유 구간도 과도하게 가로로 퍼져 보이지 않도록 최소 1분 시간창을 적용한다.
  - 표시 캔버스는 최대 760px 폭으로 제한한다.
