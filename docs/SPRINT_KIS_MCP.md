...
- 주문 `POST`는 서버 접수 여부가 불명확한 상태에서 자동 재전송하면 중복 주문 위험이 있으므로,
  HTTP 429·`EGW00201` 응답을 포함해 공용 REST 계층에서 자동 재시도하지 않는다. 이는 `REAL`에도
  적용되는 주문 안전성 변경이며, 주문별 체결·미체결 조회와 정합성 복구 절차에서 후속 처리한다.
