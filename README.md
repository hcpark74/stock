...
| 거래 횟수 | 하루 최대 한 종목 |
| 선호 갭 | 전일 종가 대비 `+2.5% ~ +8%` |
| 고갭 조건 | `+8% ~ +10%`는 예상 체결대금 50억원 이상만 허용 |
...
flowchart TD
    ...
    B -->|8% 이상 10% 미만| D{예상 체결대금 50억원 이상?}
...
10% 미만이면서 예상 체결대금이 50억원 이상일 때만 가능합니다. 주문 직전 조건이 깨지면 매수하지 않습니다.
