...
자산 메뉴와 주문 메뉴는 데이터의 소유 기준으로 구분합니다. 자산 메뉴는 KIS 잔고/계좌 응답에서 온 "현재 계좌 상태"를 보여주고, 주문 메뉴는 F3/F4/F5가 발생시킨 "주문 행위와 처리 결과"를 보여줍니다. `주문가능금액`은 자산 데이터이므로 자산 메뉴가 원천이며, 주문 메뉴에서는 주문 판단용 보조 참조값으로만 노출합니다. 자산 조회 성공 결과는 `asset_snapshots` DB 테이블에도 저장해 감사/장애 분석용 이력으로 남깁니다.
