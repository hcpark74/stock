...
| **전일 포지션 잔류 의심** (`STALE_POSITION_DETECTED`) | 전일 상태가 보유/진입 중(`HOLDING`/`ENTERING`)이면 긴급 알림과 함께 **당일 자동 진입을 차단**하고 나머지 기동은 계속. 정상 청산(`CLOSED`) 상태 파일이면 알림만 보내고 당일 거래 진행 | 계좌 보유 수량과 미체결 주문을 확인. 문제가 없으면 `data/state/today_state.json` 삭제 후 재시작하면 차단이 해제됨. 잔고가 실제로 남아 있으면 수동 정리 먼저 |
...
- 보유 후 가격흐름 차트는 서버의 최근 tick 버퍼(최대 5,000건)를 원천으로 원시 tick 선을 그립니다. 보유 중에는 `[max(진입시각, 지금-20분) ~ 지금]` 슬라이딩 창, 청산(CLOSED) 후에는 진입~마지막 체결 전체 구간을 고정해 표시합니다. 눈금 간격은 창 크기에 따라 1/2/5/10분, 시간 라벨 간격은 겹치지 않도록 화면 너비 기준으로 1·2·5·10·15·30분 중 자동 선택됩니다. 캔버스 폭은 CSS `width:100%`를 따릅니다.
...
- 청산 시 `state.set_closed`는 tick 이력을 지우지 않으며, 다음 거래일 시작(`_clear_for_trading_day`) 때 정리됩니다.
