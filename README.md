...
- **원금 손실 위험**: 자동매매는 수익을 보장하지 않습니다. 갭업 종목은 시장에서 변동성이 가장 큰 종목군입니다. Hard Stop(-2.0%)과 Step Trailing(-2.0%)이 기본 방어선이지만, 급락·하한가·VI(변동성완화장치) 발동·시장가 슬리피지 상황에서는 실제 손실이 설정값보다 커질 수 있습니다.
...
# ── F1: 후보 종목 선정 조건 ────────────────────
# 갭(전일 종가 대비 시초가 상승률) 허용 범위: 2.5% 이상만 후보,
# 2.5~8%가 선호 구간, 10% 이상은 과열로 제외
F1_GAP_MIN=0.025
...
# 기존 1.5%와 현재 2.0% 트레일 청산을 틱 단위로 비교 기록
TRAILING_SHADOW_ENABLED=1
TRAILING_SHADOW_BASELINE_TRAIL=0.015
# WS 무틱/회복 반복 로그 쿨다운(실제 연결 끊김은 즉시 별도 경고)
...
# 저우선순위 요청 승격 기준. 초가 아니라 현재 레이트의 슬롯 수
...
docs/
  ...
  DB_DESIGN.md
  TABLE_DESIGN.md
  PAPER_STRATEGY_IMPROVEMENT_PLAN.md  # PAPER 데이터·통계 기반 전략 개선 절차
  SPRINT.md
  html/                 # Web UI 정적 파일
tests/
```
